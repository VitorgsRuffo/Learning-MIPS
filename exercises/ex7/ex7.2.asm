	#Nome: Vitor G. S. Ruffo | #Atividade 12.2

.data
	vetor:		.space 400
	buffer: 		.space 100
	caminhoArquivo: 	.asciiz "dados.txt" #arquivo com inteiros separador por um espaço em branco, e.g: 1 2 3 4
	...
	main:
		...
		converter_buffer_para_vetor: #((a0) buffer:address, (a1) vetor:address):int
			...
			loop_converter:
				...
				beq $t6, $t3, ultimo_caracter_new_line #if(buffer[j] == '\n') goto ...
				beq $t6, $zero, ultima_conversao #if(buffer[j] == '\0') goto ...
		
				bne $t6, $t4, inc_j #if(buffer[j] != ' ') goto inc_j
		...
		contar_caracteres_buffer: #((a0) buffer:address):int
			...
			loop_contar_buffer:
				...
				beq $t3, $t4, fim_contar_buffer
				beq $t3, $zero, fim_contar_buffer #if (t3 == '\0' || t3 == '\n') sair do loop
